...
export async function POST(request: NextRequest) {
  try {
    ...
    const priceId =
      duration === "30"
        ? process.env.NEXT_PUBLIC_STRIPE_ANONYMOUS_30_PRICE_ID
        : process.env.NEXT_PUBLIC_STRIPE_ANONYMOUS_60_PRICE_ID;

    // Create a one-time payment session for anonymous calls
